...
\frametitle{Our Contributions: Walking Onions}
  \begin{itemize}
    \item \textbf{Improved Scalability.} Adding new relays to the network while
      clients maintain constant-size bandwidth/cpu overhead.
    \item[]~
    \item \textbf{Same Thread Model.} Maintains the existing security model for Tor in one
      variant, with a slight loss of forward secrecy in the other.
    \item[]~
    \item \textbf{Performance Improvements Today.} Demonstrates improvements at
      networks the size of Tor today.
  \end{itemize}
\end{frame}
...
\frametitle{Outline}
  \begin{itemize}
    \item Overview of current Tor and performance bottlenecks
    \item Overview of Walking Onions
      \begin{itemize}
        \item Two variants
        \item Comparison to current Tor
      \end{itemize}
    \item Performance evaluation against current Tor
      \begin{itemize}
        \item Bandwidth
        \item CPU
      \end{itemize}
