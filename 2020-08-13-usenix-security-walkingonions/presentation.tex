...
\begin{frame}
\frametitle{Motivation: Safely Enable Scalability of Anonymity Networks like Tor }
  \begin{itemize}
    ...
    \item[]~
    \item \textbf{Safely.} Maintain the existing security model of Tor.
  \end{itemize}
\end{frame}
...
\begin{tikzpicture}
  ...
  \end{pgfonlayer}
    ...
    {\textbf{Extend} $R_5$ $\pgfuseimage{clikey}_1$};
    ...
    {\textbf{Extend} $R_3$ $\pgfuseimage{clikey}_1$};
...
\frametitle{New Data Structures: SNIPs and ENDIVEs}
  ...
  Goal: Relay entries that are:
  \begin{itemize}
    \item[]~
    \item Individually verifiable without requiring the entire consensus.
    \item[]~
    \item Identifiable by a single index.
  \end{itemize}
\end{frame}
...
\begin{tikzpicture}
  ...
  \begin{pgfonlayer}{backbackground}
    ...
    \draw<1>[line width=1pt,->] (user1.east) -- (R1.west) node [below, midway]
    {Create $\pgfuseimage{clikey}_1, \pgfuseimage{clivrfkey}_1$};

    \draw<2>[line width=1pt,->] (R1.east) -- (R5.west) node [below, midway]
    {Extend $\pgfuseimage{clikey}_1, \pgfuseimage{clivrfkey}_1$};
  ...
  \begin{pgfonlayer}{background}
    \node<2> [fill=yellow,draw=red,ultra thick,font=\large, text width=.5\textwidth,anchor=north west,ellipse callout,text centered,callout absolute pointer = {($(R1.north)+(0,0)$)}] at (2.5,3) {$k \leftarrow DH(\pgfuseimage{clikey},\pgfuseimage{srvkey}_1)$\\ $r \leftarrow VRF_{\pgfuseimage{srvvrfkey}_1}(DH(\pgfuseimage{clivrfkey},\pgfuseimage{srvkey}_1))$ };
  \end{pgfonlayer}
