...
\frametitle{Motivation}
  \begin{itemize}
    \item<1-> \textbf{Enable Scalability.} Ensure that client and relay growth
...
\begin{tikzpicture}
  ...
  \end{pgfonlayer}
    ...
    \node<1->[anchor=south,inner sep=0pt] (d4) at ($(cons.south
    west)!(d2.south)!(cons.south east)$) {\shortdocused};
    ...
    \node<3->[anchor=north,inner sep=0pt] (d3) at (d2.south) {\shortdocused};
    ...
    \node<7->[anchor=north,inner sep=0pt] (d2) at (d1.south) {\shortdocused};
...
\frametitle{Tor Security Model: Security over Scalability}
  \begin{itemize}
    ...
    \item<1->[]~
    \item<2-> \textbf{Goal:} Same protections against eptistemic and route capture
...
\begin{tikzpicture}
  ...
  \begin{pgfonlayer}{backbackground}
    \node [anchor=north,inner sep=0pt] (cons) at (8, -1.2)  {\scalebox{.5} {
      \longconsensus} };
    \node [anchor=north,inner sep=0pt] (netdoc) at ($(cons.north east)+(1,0)$)
    {\scalebox{0.5}{\netdocunsigned }};
    \node [anchor=north,inner sep=0pt] (d1) at ($(netdoc.south)+(0,-.1)$)
    {\scalebox{0.5}{\shortdoc}};
    \node [anchor=north,inner sep=0pt] (d2) at (d1.south)
    {\scalebox{0.5}{\shortdoc}};
    \node [anchor=north,inner sep=0pt] (d3) at (d2.south)
    {\scalebox{0.5}{\shortdoc}};
    \node [anchor=south,inner sep=0pt] (d4) at ($(cons.south
    west)!(d2.south)!(cons.south east)$) {\scalebox{0.5}{\shortdoc}};
    \node [anchor=north,inner sep=0pt] at ($(d3.south)+(0,.1)$) {$\mathbf{\vdots}$};
    \draw [line width=2pt,double,thick,double equal sign distance,-implies] ($($(netdoc.north west)!(cons.east)!(netdoc.south west)$)!.1!(cons.east)$) -- ($($(netdoc.north west)!(cons.east)!(netdoc.south west)$)!.8!(cons.east)$);
  \end{pgfonlayer}
...
\frametitle{Takeaways}
  \begin{itemize}
    \item<1-> The design of Tor today imposes impractical overheads to clients as
      the network scales.
    \item<1->[]~
    \item<2-> Walking Onions removes the per-relay bandwidth and storage cost to
      clients.
    \item<2->[]~
    \item<3-> Walking Onions offers the same security protections against epistemic
