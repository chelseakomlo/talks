...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item[]
    \item<3-> "On the (in)security of ROS": Benhamouda et al. showed that ROS attacks can be performed in polynomial time. 
    \item[]
    \item<3-> A forgery for the trivial construction can be produced using this ROS solver, by finding 
  \end{itemize}
	\[ c^*= H(R^*, Y, m^*) \uncover<2->{ = \sum_{j=1}^{k} H(R_{ij}, Y, m_j)  =
	\sum c_j  \text{ for some } (R_{ij}, m_j), \dots }, \text{such that} \]

	\[ \uncover<4-> {z_i^* = \sum r_{ij} + \lambda_i \cdot \sk_i \cdot \sum c_j} = \uncover<5->
