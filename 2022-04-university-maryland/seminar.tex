...
\procedure{Single-Party EdDSA Signing and Verification}{
  ...
  \uncover<1-> { (\sk, \pk) \leftarrow \mathit{KeyGen()} \\ }
  \uncover<2-> { \< \sendmessageleft{top=\text{$m$}} \< \\ }
  \uncover<3-> { r \gets H(\sk, m)  } \\
  \uncover<3-> { \text{\algcom Deterministic to mitigate bad randomness} } \\
  \uncover<4-> { R = g^r ;\ c = H(R, \pk, m) \\ }
  \uncover<5-> { z = r + c \cdot \sk \\ }
  \uncover<6-> { \< \sendmessageright{top=\text{$(m, \sigma=(R, z))$}} \< \\ }
  \< \< \uncover<7-> { c = H(R, \pk, m) \\ }
  \< \< \uncover<8-> { R' = g^z \cdot \pk^{-c} \\ }
  \< \< \uncover<9-> { \text{Output } 1 \text{ if } R \stackrel{?}{=} R' \\ }
  \< \< \uncover<9-> { \text{Otherwise, output } 0 \\ }
}
}
\end{frame}
...
\scalebox{0.9} {
  ...
  \uncover<1-> { r_i \random \mathbb{Z}_q^*;\ R_i \gets g^{r_i} \\ }
...
\begin{frame}
  ...
  \end{itemize}
	...
	\uncover<3-> { Allowing for the forgery for participant $i$:}
