...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<4-> However, given a random oracle $H\ : \{0,1 \}^n \rightarrow \Zq$, the \textbf{ROS problem} asks to find a set $\langle c_1, \ldots, c_\ell \rangle$  and $c^*$ such that 
	    \[  \sum_{i=1}^\ell c_i = c^*  \]

    \item<3-> "On the (in)security of ROS": Benhamouda et al. showed that the ROS problem can be solved in polynomial time. 
  \end{itemize}
\end{frame}
